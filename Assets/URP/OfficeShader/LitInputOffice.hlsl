//输入各种数据
#ifndef UNIVERSAL_LIT_INPUT_INCLUDED //为了防止重复包含该文件而设置的宏定义
...
//URP中定义纹理的方式如下，无需写在CBUFFER中。但是如果需要纹理的Scale、Offset属性，需要在CBUFFER中定义一个float4的_ST属性
//采样器可以定义纹理的Wrap Mode(重复模式)、Filter Mode(过滤模式)
//_BaseMap、_BumpMap、_EmissionMap没有在这里定义，是因为已经在SurfaceInput.hlsl中定义了
...
//决定了不同工作流中对不同纹理的采样
//如果选择的Specular工作流，那么就是对_SpecGlossMap纹理进行采样
//如果选择的是Metallic工作流，那么就是对_MetallicGlossMap纹理进行采样
//这是个宏定义，把两种采样都定义为名称SAMPLE_METALLICSPECULAR(uv)，方便其他地方调用；其他地方就不用再判断宏了
...
{
    //在Metallic工作流中：R表示金属度、A为光滑度（GB通常未使用）
    //在Specular工作流中：RGB为高光色、A为光滑度
    half4 specGloss;
    
#ifdef _METALLICSPECGLOSSMAP //如果启用了金属/高光贴图，那么走贴图采样的逻辑
    ...
    #endif
#else // _METALLICSPECGLOSSMAP //未启用金属/高光贴图，那么走材质参数
    #if _SPECULAR_SETUP //如果启用高光
        specGloss.rgb = _SpecColor.rgb;//Specular模式，没有高光贴图时，高光色直接使用材质颜色
    #else
        specGloss.rgb = _Metallic.rrr;//Metallic模式，金属度是标量，把_Metallic的R通道赋值，通常只使用R通道
    #endif
