//Chapter07_切线空间下的法线贴图
Shader "Unity Shaders Book/Chapter07-NormalMapTangentSpace"
{
    Properties
    {
        //基础颜色
    ...
    SubShader
    {
        Pass
        {
            ...
            sampler2D _BumpMap;
            //纹理的缩放（Scale）和平移（Translation），所以后缀加_ST，是Unity规定的标准写法，不是随意起名的
            //其中_BumpMap_ST.xy代表缩放值，_BumpMap_ST.zw代表偏移值
            float4 _BumpMap_ST;
            float _BumpScale;
            fixed4 _Specular;
            float _Gloss;
            ...
            v2f vert (a2v v)
            {
                ...
                //得到从模型空间到切线空间的变化矩阵rotation，下面就可以直接用这个变量。
